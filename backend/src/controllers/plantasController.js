...
// ---- PUXA AS PLANTAS QUE PERTENCEM AO USER ----
...
// ---- DELETA UMA PLANTA DO USER PELO ID ---
export const delPlantaUsuario = async (req, res) => {
    try {
        const { id } = req.params; // id da planta passada na url
...
// ---- ATUALIZA UMA PLANTA DO USER PELO ID ----
export const updatePlantaUsuario = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const { apelido, ambiente } = req.body;
        ...
        return res.status(200).json({ message: "Dados da planta atualizados com sucesso!" });
    } catch (err) {
        console.error("Erro ao tentar editar planta:", err);
        return res.status(500).json({ error: "Erro interno ao tentar editar planta." });
    }
